# !pip install tensorflow==1.15.0 tensorflow-probability==0.8.0 hdbscan umap-learn scikit-learn
...
class MDN:
    ...
    def plot_distribution_fit(self, n_samples_batch = 1, alpha = 0.2, y_scaler = None):
        ...
        for col in y_sampled.columns:
            sns.kdeplot(y_sampled[col], fill=True, alpha = alpha)      
        sns.kdeplot(self._y.ravel(), fill=True, linewidth = 2.5, label = 'True dist')
        plt.legend()
    ...
    def plot_all_distribution_fit(self, n_samples_batch = 1, alpha = 0.2, y_scaler = None):  
        ...
        i = 0
        for elem in all_preds:
            sns.kdeplot(elem['Y_PRED_MEAN'], fill=True, alpha = 0.15, label = 'fitted_mixture_' + str(i))      
            i = i +1
        sns.kdeplot(self._y.ravel(), fill=True, label = 'True dist')
        plt.legend()
    ...
    def plot_samples_vs_true(self, X_pred, y_pred, alpha = 0.4, non_linear = False, y_scaler = None):
        ...
        plt.scatter(X_1d, y_sampled, alpha = alpha, label = 'Generated sample', edgecolors='black', s=10)
        plt.scatter(X_1d, y_pred, alpha = alpha, label = 'True',  s=1, color='red')
        plt.legend()
